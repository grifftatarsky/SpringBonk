...
public class MeController
{
    ...
    public UserInfoDto getMe(Authentication auth)
    {
        if (auth instanceof JwtAuthenticationToken jwtAuth)
        {
            ...
            final var exp = Optional.ofNullable(jwtAuth.getTokenAttributes()
                                                       .get(JwtClaimNames.EXP)).map(expClaim -> switch (expClaim)
            {
                case Long longEXP -> longEXP;
                case Instant instantEXP -> instantEXP.getEpochSecond();
                case Date dateEXP -> dateEXP.toInstant().getEpochSecond();
    ...
    /**
     * @param username
     *     a unique identifier for the resource owner in the token (sub.claim by default)
     * @param email
     *     OpenID email claim
     * @param roles
     *     Spring authorities resolved for the authentication in the security context
     * @param exp
     *     seconds from 1970-01-01T00:00:00Z UTC until the specified UTC date/time when the access token expires
     */
