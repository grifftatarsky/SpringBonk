...
/**
 * Shown when the federated sticker remote can't be loaded (the micro-frontend
 * is down or unreachable). Routed in as a fallback by app.routes when
 * loadRemoteModule rejects.
 */
@Component({
  selector: 'app-jpss-unavailable',
  imports: [RouterLink],
  ...
  template: `
    <div class="grid min-h-[60vh] place-items-center px-4 py-16 text-center text-fg">
      <div class="max-w-md">
        ...
        <h1 class="mt-6 text-2xl font-semibold tracking-tight">Jo Peace Stickers is currently unavailable</h1>
        <p class="mt-2 text-sm text-fg-muted">
          …but the globe will be back! Nobody has moved any of the photos.
        </p>
        <a routerLink="/"
